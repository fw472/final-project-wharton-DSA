...
# (optionc dataset, county only, 10 pre‑specified predictors)
# Hyperparameter tuning: 5 mtry × 5 nodesize
#
# Run:
#   source("random_forest_10features.R")
#   Rscript random_forest_10features.R
#
# Inputs:
#   spine_data_optionc.csv   (county)
#
# Outputs:
#   figures/random_forest_optionc_county_10features/
#
# Predictors:
#   Proprietor share, net migration rate, pre‑crisis unemployment rate,
#   working‑age share, median household income, poverty rate,
#   log population, log population density, population trend, RUCC (factor)
#
# Employment variables ARE included (pre‑unemployment is one of the 10).
# Categorical predictors (RUCC) are treated as factors.
# Optimisation: hyperparameter tuning (5 mtry × 5 nodesize), train/validation loss, Q‑Q residuals.
...
county_output <- file.path(project_dir, "figures",
                           "random_forest_optionc_county_10features")
...
# ---- Data preparation (county, optionc, 10 fixed predictors) ----------------
prepare_recovery_data <- function(data, crisis) {
  ...
  # Exactly 10 pre‑crisis predictors
  rural_var <- if (suffix == "2007") "rucc_2003_2007" else "rucc_2013_2019"

  selected_vars <- c(
    paste0("poverty_rate_", suffix),
    paste0("median_hh_income_", suffix),
    paste0("total_population_", suffix),
    rural_var,
    paste0("population_density_", suffix),
    paste0("proprietor_share_", suffix),
    paste0("net_migration_rate_", suffix),
    paste0("pre_unemployment_", suffix),
    paste0("working_age_share_", suffix),
    paste0("pop_trend_", suffix)
  )

  missing_vars <- setdiff(selected_vars, names(data))
  if (length(missing_vars)) {
    stop("Missing predictor columns in data: ", paste(missing_vars, collapse = ", "))
  }
  ...
  # Give uniform names across crises
  renamed <- c(
    "poverty_rate", "median_hh_income", "total_population", "rurality",
    "population_density", "proprietor_share", "net_migration_rate",
    "pre_unemployment", "working_age_share", "pop_trend"
  )
  names(result)[match(selected_vars, names(result))] <- renamed
  ...
  # Log transforms for population and density
  result <- result %>%
    mutate(
      log_population = if_else(total_population > 0, log(total_population), NA_real_),
      log_density    = if_else(population_density > 0, log(population_density), NA_real_)
    ) %>%
    select(-total_population, -population_density)

  # RUCC as unordered factor
...
tune_forest <- function(training, formula, predictor_count, seed) {
  ...
  results <- purrr::pmap_dfr(tuning_grid, function(mtry, nodesize) {
    ...
    tibble(
      ...
      val_rmse   = sqrt(oob_mse)
    )
  })
  list(
    best    = results %>% slice_min(val_rmse, n = 1, with_ties = FALSE),
    results = results
  )
}
...
# Partial dependence (top 4 numeric predictors, excluding factor rurality)
...
predictions    <- prediction_table  # already filtered for County
...
p_imp <- ggplot(importance_data,
  ...
  labs(title = "County variable importance (10 features)",
...
p_pd <- ggplot(partial_data,
  ...
  labs(title = "County response profiles (10 features)",
...
p_oob <- ggplot(errors, aes(trees, oob_mse, color = crisis)) +
  ...
  facet_wrap(~crisis, scales = "free_y") +
  scale_color_manual(values = crisis_colors) +
  labs(title = "County convergence (10 features)",
...
p_met <- ggplot(metrics, aes(crisis, value, fill = crisis)) +
  ...
  labs(title = "County performance (10 features)",
...
save_plot_gallery(plots, county_output, "county_random_forest_10features.pdf")
...
saveRDS(map(rf_models, "model"),
        file.path(county_output, "fitted_random_forest_10features.rds"))

message("County 10‑feature random forest analysis complete (5x5 tuning).\nOutputs: ",
